...
%     FIGURE=figure;
try
    hue_values_along_line=zeros(1,Width);
    for x=1:Width
         y=round(polyval(P,x));
        if y>=1 && y<=Height
            hue_values_along_line(x)=img(y,x,1)*360;
        else
            hue_values_along_line(x)=-1;
        end
    end

    similarity_along_line = zeros(1,Width);
    for x=1:Width
         [n1, n2] = find_similar(hue_values_along_line,x,similarity_thres,0);
         similarity_along_line(x) = min(n1,n2);
    end
%     plot (1:Width,similarity_along_line);
    hold on;
    [pks,locs,~,~] = findpeaks(similarity_along_line);
%     scatter(locs,pks);
%     plot (1:Width,hue_values_along_line,'color',[0,0,0]);
    hold on;

    valid_color_pts=zeros(1,Width);
    valid_pts_no=0;
    prev_valid_pt=0;
    for i=1:numel(locs)
